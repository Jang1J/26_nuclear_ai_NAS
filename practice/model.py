...
def build_mlp(input_dim, n_classes, dropout_rate=0.3):
    ...
    return models.Sequential([
        ...
        layers.Dense(512, activation="relu", kernel_initializer=HE),
...
def build_cnn(window_size, n_features, n_classes, dropout_rate=0.3):
    ...
    branch_a = layers.Conv1D(64, kernel_size=1, padding="same", activation="relu", kernel_initializer=HE)(inputs)
    branch_b = layers.Conv1D(64, kernel_size=2, padding="same", activation="relu", kernel_initializer=HE)(inputs)
    branch_c = layers.Conv1D(64, kernel_size=3, padding="same", activation="relu", kernel_initializer=HE)(inputs)
    ...
    # 채널 통합
    x = layers.Conv1D(256, kernel_size=1, activation="relu", kernel_initializer=HE)(x)
    x = layers.BatchNormalization()(x)
    x = layers.Dropout(dropout_rate)(x)
...
def build_cnn_attention(window_size, n_features, n_classes, dropout_rate=0.3, num_heads=4):
    ...
    x = layers.Conv1D(128, kernel_size=1, padding="same", activation="relu", kernel_initializer=HE)(inputs)
    x = layers.BatchNormalization()(x)
    x = layers.Conv1D(128, kernel_size=3, padding="same", activation="relu", kernel_initializer=HE)(x)
    x = layers.BatchNormalization()(x)
    x = layers.Dropout(dropout_rate)(x)
...
def _tcn_residual_block(x, n_filters, kernel_size, dilation_rate, dropout_rate):
    ...
    if x.shape[-1] != n_filters:
        shortcut = layers.Conv1D(n_filters, kernel_size=1, kernel_initializer=HE)(x)
    else:
        shortcut = x
...
def build_transformer(window_size, n_features, n_classes, dropout_rate=0.3,
    ...
    - ff_dim = 4 × d_model (표준 Transformer 비율, H2 fix)
    ...
    ff_dim = 4 * d_model  # 208 × 4 = 832 (표준 비율)
    ...
    x = layers.Dense(d_model, kernel_initializer=HE)(inputs) + pos_embedding(positions)
    ...
    # Feed-forward network (H2: ff_dim = 4 × d_model = 832)
    ff_output = layers.Dense(ff_dim, activation="relu", kernel_initializer=HE)(x)
    ff_output = layers.Dropout(dropout_rate)(ff_output)
    ff_output = layers.Dense(d_model, kernel_initializer=HE)(ff_output)
